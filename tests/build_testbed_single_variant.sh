...
# By default <algo> is OQS_SIG_alg_sphincs_haraka_128f_robust. To set it differently, just pass the OQS_SIG_alg_sphincs_variant name as the first parameter
# Use one parameter to choose the algorithm to test
# Use no parameters to use default algorithm to test

# To test this variant run the run_testbed.sh script
...
if [ $# -eq 1 ]
then
	echo "Build with this algorithm as default: "
	algo=$1
else
	echo "Build with default algorithm: "
...
cmake -GNinja -DCMAKE_INSTALL_PREFIX=../../openssl_pq/oqs .. -DOQS_SIG_DEFAULT="$algo" >> /dev/null
ninja >> /dev/null
